(function(){
  ...
  const POPOVER_IDS = ["settingsPopover", "pendingPopover", "indicatorPopover", "clearPopover", "toolStylePopover", "authPopover"];
  ...
  /* ============================================================
     FIREBASE AUTH — Google + Email/Kata Sandi
     Popover ini dipicu dari tombol "Trader" di topbar (id="authBtn").
     Login sepenuhnya opsional untuk memakai chart/replay/backtest —
     hanya menentukan apa yang tampil di chip akun.
     ============================================================ */
  const firebaseConfig = {
    apiKey: "AIzaSyDj1YOOGPvxGRwxy5Ecv2OVn-I3ClFKowY",
    authDomain: "web-backtest.firebaseapp.com",
    projectId: "web-backtest",
    storageBucket: "web-backtest.firebasestorage.app",
    messagingSenderId: "183370830227",
    appId: "1:183370830227:web:220338047a73a470045866",
    measurementId: "G-X7JKJZ1CEN"
  };
  let fbAuth = null;
  if (typeof firebase !== "undefined" && firebase.initializeApp){
    ...
    }catch(err){ console.error("Firebase gagal diinisialisasi:", err); }
  } else {
    console.warn("Firebase SDK belum termuat (cek koneksi internet) — tombol login akan menampilkan pesan error saat dipakai.");
  }
  ...
  function authTranslateError(err){
    ...
    const map = {
      ...
      "auth/user-not-found": "Email belum terdaftar. Coba klik Daftar.",
      "auth/wrong-password": "Kata sandi salah.",
      "auth/email-already-in-use": "Email sudah terdaftar. Coba Masuk saja.",
      "auth/weak-password": "Kata sandi minimal 6 karakter.",
      "auth/invalid-credential": "Email atau kata sandi salah.",
      "auth/missing-password": "Isi kata sandi dulu.",
      ...
      "permission-denied": "Akses ditolak — cek Firestore Security Rules di Firebase Console (izinkan user membaca/menulis dokumennya sendiri di users/{uid}/backtests).",
      "unavailable": "Server cloud sedang tidak bisa dijangkau. Coba lagi.",
      "unauthenticated": "Kamu belum masuk. Silakan login dulu."
    };
    return map[code] || (err && err.message) || "Terjadi kesalahan. Coba lagi.";
  }
  function updateAuthUI(user){
    const loggedOut = el("authLoggedOut");
    const loggedIn = el("authLoggedIn");
    ...
    if (user){
      ...
      btn.title = user.email || user.displayName || "Akun";
      const name = user.displayName || (user.email ? user.email.split("@")[0] : "Trader");
      label.textContent = " " + name;
      el("authUserName").textContent = user.displayName || name;
  ...
  el("authBtn").addEventListener("click", (e) => {
    e.stopPropagation();
    const pop = el("authPopover");
    const willOpen = pop.classList.contains("hidden");
    closeAllPopovers("authPopover");
    if (willOpen) authClearError();
    pop.classList.toggle("hidden");
  });
  document.addEventListener("click", (e) => {
    const pop = el("authPopover");
    if (pop.classList.contains("hidden")) return;
    if (e.target.closest("#authPopover") || e.target.closest("#authBtn")) return;
    pop.classList.add("hidden");
  });
  el("authGoogleBtn").addEventListener("click", () => {
    if (!fbAuth){ authShowError("Login belum siap — cek koneksi internet lalu muat ulang halaman."); return; }
    authClearError();
    const provider = new firebase.auth.GoogleAuthProvider();
    fbAuth.signInWithPopup(provider)
      .then(() => { el("authPopover").classList.add("hidden"); })
      .catch((err) => { if (err.code !== "auth/popup-closed-by-user") authShowError(authTranslateError(err)); });
  });
  el("authSignInBtn").addEventListener("click", () => {
    if (!fbAuth){ authShowError("Login belum siap — cek koneksi internet lalu muat ulang halaman."); return; }
    const email = el("authEmail").value.trim();
    const pw = el("authPassword").value;
    if (!email || !pw){ authShowError("Isi email dan kata sandi dulu."); return; }
    authClearError();
    fbAuth.signInWithEmailAndPassword(email, pw)
      .then(() => { el("authPopover").classList.add("hidden"); el("authEmail").value=""; el("authPassword").value=""; })
      .catch((err) => authShowError(authTranslateError(err)));
  });
  el("authSignUpBtn").addEventListener("click", () => {
    ...
    const pw = el("authPassword").value;
    if (!email || !pw){ authShowError("Isi email dan kata sandi dulu."); return; }
    if (pw.length < 6){ authShowError("Kata sandi minimal 6 karakter."); return; }
    authClearError();
  ...
  el("authSignOutBtn").addEventListener("click", () => {
    if (!fbAuth) return;
    fbAuth.signOut().then(() => { el("authPopover").classList.add("hidden"); });
  });
  [el("authEmail"), el("authPassword")].forEach(inp => {
    inp.addEventListener("keydown", (e) => { if (e.key === "Enter") el("authSignInBtn").click(); });
  });

  let fbDb = null;
  if (fbAuth && typeof firebase !== "undefined" && firebase.firestore){
    try{ fbDb = firebase.firestore(); }catch(err){ console.error("Firestore gagal diinisialisasi:", err); }
  }
  ...
  const ICONS = {
    ...
    cloud: '<path d="M7 18a4.5 4.5 0 0 1-.7-8.94A5.5 5.5 0 0 1 17.2 8.06 4 4 0 0 1 17 18H7Z"/>',
    google: '<path d="M21.35 11.1h-9.17v2.98h5.24c-.23 1.4-1.6 4.1-5.24 4.1-3.15 0-5.72-2.6-5.72-5.8s2.57-5.8 5.72-5.8c1.8 0 3 .77 3.68 1.43l2.5-2.42C16.9 3.99 14.7 3 12.18 3 7.13 3 3 7.03 3 12s4.13 9 9.18 9c5.3 0 8.82-3.72 8.82-8.96 0-.6-.07-1.06-.15-1.94Z"/>',
...
download: '<path d="M12 3v12M7 10l5 5 5-5"/><path d="M4 19h16"/>',
  ...
  document.querySelectorAll(".bt-subtab[data-bt]").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      ["posisi","riwayat","cloud","atur"].forEach(t => el("btPanel" + t[0].toUpperCase()+t.slice(1)).classList.toggle("hidden", t !== currentBtTab));
      renderBacktestBody();
    });
  });
  ...
  function renderCloudPanel(){
    const wrap = el("btPanelCloud");
    const user = fbAuth && fbAuth.currentUser;
    if (!user){
      wrap.innerHTML = `<div class="cloud-login-prompt">
        <div>Masuk dulu untuk menyimpan &amp; melihat hasil backtest kamu di cloud, bisa diakses dari perangkat mana saja.</div>
        <button id="cloudLoginPromptBtn" class="place-btn">Masuk / Daftar</button>
      </div>`;
      const btn = el("cloudLoginPromptBtn");
      if (btn) btn.addEventListener("click", (e) => {
        e.stopPropagation();
        closeAllPopovers();
        el("authPopover").classList.remove("hidden");
      });
      el("btCloudCount").textContent = "0";
      return;
    }
    if (!fbDb){
      wrap.innerHTML = '<div class="term-empty">Cloud belum siap — cek koneksi internet lalu muat ulang halaman.</div>';
      return;
    }
    wrap.innerHTML = '<div class="term-empty">Memuat riwayat cloud…</div>';
    fbDb.collection("users").doc(user.uid).collection("backtests")
      .orderBy("savedAt", "desc").limit(30).get()
      .then(snap => {
        if (currentBtTab !== "cloud") return; // user switched tabs before this resolved
        el("btCloudCount").textContent = String(snap.size);
        if (snap.empty){
          wrap.innerHTML = '<div class="term-empty">Belum ada hasil backtest yang disimpan. Klik "Simpan ke Cloud" di atas.</div>';
          return;
        }
        let html = "";
        snap.forEach(doc => {
          const d = doc.data();
          const s = d.stats || {};
          const savedAt = d.savedAt && d.savedAt.toDate ? d.savedAt.toDate() : null;
          const dateTxt = savedAt ? fmtDateFull(savedAt.getTime()) : "—";
          const totalR = typeof s.totalR === "number" ? s.totalR : 0;
          const rTxt = (totalR>=0?"+":"") + totalR.toFixed(2) + "R";
          const rCls = totalR >= 0 ? "pos" : "neg";
          const winRate = typeof s.winRate === "number" ? s.winRate.toFixed(0) : "0";
          html += `<div class="rw-card cloud-card">
            <span class="rw-badge ${totalR>=0?'buy':'sell'}">${d.symbol||"—"}</span>
            <div class="rw-mid">
              <div class="rw-sym">${d.timeframe||""} <span class="rw-tf">${d.tradeCount||0} trade · WR ${winRate}%</span></div>
              <div class="rw-range">${dateTxt}</div>
            </div>
            <div class="rw-right">
              <div class="rw-r ${rCls}">${rTxt}</div>
            </div>
            <button class="rw-del" data-clouddel="${doc.id}" title="Hapus dari cloud">${iconSvg("close",12)}</button>
          </div>`;
        });
        wrap.innerHTML = html;
        wrap.querySelectorAll("[data-clouddel]").forEach(b => b.addEventListener("click", () => {
          if (!confirm("Hapus hasil backtest ini dari cloud?")) return;
          const id = b.dataset.clouddel;
          fbDb.collection("users").doc(user.uid).collection("backtests").doc(id).delete()
            .then(() => renderCloudPanel())
            .catch(err => alert("Gagal menghapus: " + (err && err.message ? err.message : err)));
        }));
      })
      .catch(err => {
        if (currentBtTab !== "cloud") return;
        wrap.innerHTML = `<div class="term-empty">Gagal memuat riwayat cloud: ${authTranslateError(err)}</div>`;
      });
  }

  function btCloudStatus(msg, ok){
    const box = el("btCloudSaveStatus");
    box.textContent = msg;
    box.classList.remove("ok","err");
    box.classList.add(ok ? "ok" : "err", "show");
    clearTimeout(btCloudStatus._t);
    btCloudStatus._t = setTimeout(() => box.classList.remove("show"), 4000);
  }
  el("btSaveCloudBtn").addEventListener("click", (e) => {
    e.stopPropagation();
    const user = fbAuth && fbAuth.currentUser;
    if (!user){
      closeAllPopovers();
    ...
    if (!fbDb){ btCloudStatus("Cloud belum siap — cek koneksi internet.", false); return; }
    if (history.length === 0){ btCloudStatus("Belum ada trade untuk disimpan.", false); return; }
    const scr = getActiveScreen();
    const stats = computeStats();
    const btn = el("btSaveCloudBtn");
    btn.disabled = true;
    fbDb.collection("users").doc(user.uid).collection("backtests").add({
      savedAt: firebase.firestore.FieldValue.serverTimestamp(),
      symbol: (scr && scr.symbol) || "—",
      timeframe: (scr && scr.timeframe) || "—",
      tradeCount: stats.n,
      balance: account.balance,
      stats: stats
    }).then(() => {
      btCloudStatus("Tersimpan ke cloud.", true);
      btn.classList.add("saved");
      setTimeout(() => btn.classList.remove("saved"), 1500);
      if (currentBtTab === "cloud") renderCloudPanel();
    }).catch(err => {
      btCloudStatus("Gagal menyimpan: " + authTranslateError(err), false);
  ...
  function renderBacktestBody(){
    ...
    else if (currentBtTab === "cloud") renderCloudPanel();
  ...
  function setDrawTool(tool){
    ...
    if (typeof refreshToolStyleBtn === "function") refreshToolStyleBtn();
    requestDraw();
  }
  ...
  /* ---------- toolbar "Atur Gaya Gambar" popover — edits toolDefaults[type],
     i.e. the style used for the NEXT drawing of that type. Existing drawings
     are untouched; right-click still edits those one at a time. */
  function toolStyleTypeLabel(type){
    return type === "fib" ? "Fibonacci" : type === "rect" ? "Kotak" : type === "hline" ? "Garis Horizontal" : "Trend Line";
  }
  function renderToolStyleFibLevels(target){
    if (!target.levels) target.levels = defaultFibLevels();
    const wrap = el("tsFibLevelList");
    wrap.innerHTML = target.levels.map((lv, i) => `
      <div class="fib-level-row" data-idx="${i}">
        <input type="checkbox" class="fib-en" ${lv.enabled !== false ? "checked" : ""} title="Tampilkan level ini">
        <input type="number" step="0.001" value="${lv.value}" title="Rasio (contoh: 0.618)">
    ...
    wrap.querySelectorAll(".fib-level-row").forEach(row => {
      const i = parseInt(row.dataset.idx, 10);
      row.querySelector(".fib-en").addEventListener("change", (e) => { target.levels[i].enabled = e.target.checked; saveToolDefaults(); });
      row.querySelector('input[type="number"]').addEventListener("input", (e) => {
        const v = parseFloat(e.target.value);
        if (!isNaN(v)){ target.levels[i].value = v; saveToolDefaults(); }
      });
      row.querySelector('input[type="color"]').addEventListener("input", (e) => { target.levels[i].color = e.target.value; saveToolDefaults(); });
      row.querySelector(".fib-del").addEventListener("click", () => {
        target.levels.splice(i,1);
        renderToolStyleFibLevels(target);
        saveToolDefaults();
      });
    });
  }
  function syncToolStylePopover(){
    const type = drawTool;
    const target = toolDefaults[type];
    if (!target) return; // cursor tool has no configurable style
    el("toolStyleTitle").textContent = "Gaya Gambar — " + toolStyleTypeLabel(type);
    const isFib = type === "fib";
    const isRect = type === "rect";

    el("tsColorRow").classList.toggle("hidden", isFib);
    el("tsWidthRow").classList.toggle("hidden", isFib);
    el("tsDashedRow").classList.toggle("hidden", isFib);
    if (!isFib){
      el("tsColor").value = target.color || DRAW_DEFAULT_COLOR;
      el("tsWidth").value = String(target.width || 1.4);
      el("tsDashed").checked = !!target.dashed;
    }

    el("tsRectFillRow").classList.toggle("hidden", !isRect);
    el("tsRectFillColorRow").classList.toggle("hidden", !isRect || target.fill === false);
    if (isRect){
      el("tsRectFillChk").checked = target.fill !== false;
      el("tsRectFillColor").value = target.fillColor || target.color || DRAW_DEFAULT_COLOR;
    }

    el("tsFibExtra").classList.toggle("hidden", !isFib);
    if (isFib){
      el("tsFibExtend").checked = !!target.extend;
      el("tsFibFill").checked = target.fill !== false;
      el("tsFibLabels").checked = target.showLabels !== false;
      renderToolStyleFibLevels(target);
    }
  }
  el("tsColor").addEventListener("input", () => { const t=toolDefaults[drawTool]; if(!t) return; t.color = el("tsColor").value; saveToolDefaults(); });
  el("tsWidth").addEventListener("change", () => { const t=toolDefaults[drawTool]; if(!t) return; t.width = parseFloat(el("tsWidth").value) || 1.4; saveToolDefaults(); });
  el("tsDashed").addEventListener("change", () => { const t=toolDefaults[drawTool]; if(!t) return; t.dashed = el("tsDashed").checked; saveToolDefaults(); });
  el("tsRectFillChk").addEventListener("change", () => {
    const t = toolDefaults[drawTool]; if (!t) return;
    t.fill = el("tsRectFillChk").checked;
    el("tsRectFillColorRow").classList.toggle("hidden", !t.fill);
    saveToolDefaults();
  });
  el("tsRectFillColor").addEventListener("input", () => { const t=toolDefaults[drawTool]; if(!t) return; t.fillColor = el("tsRectFillColor").value; saveToolDefaults(); });
  el("tsFibExtend").addEventListener("change", () => { toolDefaults.fib.extend = el("tsFibExtend").checked; saveToolDefaults(); });
  el("tsFibFill").addEventListener("change", () => { toolDefaults.fib.fill = el("tsFibFill").checked; saveToolDefaults(); });
  el("tsFibLabels").addEventListener("change", () => { toolDefaults.fib.showLabels = el("tsFibLabels").checked; saveToolDefaults(); });
  el("tsFibAddLevel").addEventListener("click", () => {
    const t = toolDefaults.fib;
    if (!t.levels) t.levels = defaultFibLevels();
    t.levels.push({ value: 1.618, color: "#9b59b6", enabled: true });
    renderToolStyleFibLevels(t);
    saveToolDefaults();
  });
  el("tsFibResetLevels").addEventListener("click", () => {
    toolDefaults.fib.levels = defaultFibLevels();
    renderToolStyleFibLevels(toolDefaults.fib);
    saveToolDefaults();
  });
  function refreshToolStyleBtn(){
    const btn = el("toolStyleBtn");
    const hasStyle = drawTool !== "cursor" && !!toolDefaults[drawTool];
    btn.disabled = !hasStyle;
    btn.title = hasStyle ? "Atur Gaya Gambar — " + toolStyleTypeLabel(drawTool) : "Pilih tool gambar dulu";
    if (!hasStyle) el("toolStylePopover").classList.add("hidden");
    else if (!el("toolStylePopover").classList.contains("hidden")) syncToolStylePopover();
  }
  el("toolStyleBtn").addEventListener("click", (e) => {
    e.stopPropagation();
    if (el("toolStyleBtn").disabled) return;
    const pop = el("toolStylePopover");
    const willOpen = pop.classList.contains("hidden");
    closeAllPopovers("toolStylePopover");
    if (willOpen) syncToolStylePopover();
    pop.classList.toggle("hidden");
  });
  document.addEventListener("click", (e) => {
    const pop = el("toolStylePopover");
    if (pop.classList.contains("hidden")) return;
    if (e.target.closest("#toolStylePopover") || e.target.closest("#toolStyleBtn")) return;
    pop.classList.add("hidden");
  });
  ...
  function factoryToolDefaults(){
    return {
      fib:   { color: DRAW_DEFAULT_COLOR, width: 1.4, levels: defaultFibLevels(), extend:false, fill:true, showLabels:true },
      rect:  { color: DRAW_DEFAULT_COLOR, width: 1.4, dashed:false, fill:true, fillColor: DRAW_DEFAULT_COLOR },
      hline: { color: DRAW_DEFAULT_COLOR, width: 1.4, dashed:true },
      trend: { color: DRAW_DEFAULT_COLOR, width: 1.4, dashed:false }
    };
  }
  /* ---------- per-tool default style, editable from the toolbar's "Atur Gaya
     Gambar" popover (gear icon next to Fib). Persisted so every NEW trend
     line / h-line / rectangle / Fibonacci picks up the last style the user
     configured, instead of always resetting to the hardcoded blue default.
     Existing drawings are untouched — right-click still edits those one by one. */
  const TOOL_DEFAULTS_KEY = "alhaza_tool_defaults_v1";
  function loadToolDefaults(){
    const base = factoryToolDefaults();
    try{
      const raw = localStorage.getItem(TOOL_DEFAULTS_KEY);
      if (raw){
        const parsed = JSON.parse(raw);
        ["fib","rect","hline","trend"].forEach(type => {
          if (parsed && parsed[type]) base[type] = Object.assign(base[type], parsed[type]);
        });
        if (parsed && parsed.fib && Array.isArray(parsed.fib.levels) && parsed.fib.levels.length){
          base.fib.levels = parsed.fib.levels;
        }
      }
    }catch(err){ /* ignore, fall back to factory defaults */ }
    return base;
  }
  function saveToolDefaults(){
    try{ localStorage.setItem(TOOL_DEFAULTS_KEY, JSON.stringify(toolDefaults)); }catch(err){ /* ignore */ }
  }
  let toolDefaults = loadToolDefaults();
  function deepClone(obj){ return JSON.parse(JSON.stringify(obj)); }
  // Style fields attached to every new drawing so its color/width/etc. can later be
  // customized from the right-click context menu ("Gaya Gambar"). Seeded from the
  // user's saved per-tool defaults (toolDefaults), not a fixed hardcoded style.
  function newDrawingExtra(type){
    const d = toolDefaults[type] || toolDefaults.trend;
    return deepClone(d);
  }
  ...
  /* Minimize/expand the replay panel — collapses to just play/pause + speed
     so it stops blocking the chart while scrubbing through history. State
     persists across reloads for the session (sessionStorage, not localStorage,
     since it's a per-viewing preference rather than a permanent app setting). */
  const RF_MINIMIZED_KEY = "alhaza_replay_minimized";
  function setReplayMinimized(min){
    const panel = el("replayFloat");
    panel.classList.toggle("rf-minimized", min);
    const btn = el("rfMinimizeBtn");
    btn.innerHTML = iconSvg(min ? "chevronup" : "chevrondown", 13);
    btn.title = min ? "Perbesar panel replay" : "Perkecil panel replay";
    try{ sessionStorage.setItem(RF_MINIMIZED_KEY, min ? "1" : "0"); }catch(err){ /* ignore */ }
  }
  el("rfMinimizeBtn").addEventListener("click", (e) => {
    e.stopPropagation();
    setReplayMinimized(!el("replayFloat").classList.contains("rf-minimized"));
  });
  (function initReplayMinimizedState(){
    let min = false;
    try{ min = sessionStorage.getItem(RF_MINIMIZED_KEY) === "1"; }catch(err){ /* ignore */ }
    setReplayMinimized(min);
  })();
